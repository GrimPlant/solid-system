...
def compute_fim_sos(model, dataloader, device, max_batches=16):
    ...
    for images, labels in dataloader:
        ...
        g = torch.cat(grads)
        total_g_sq_sum += torch.sum(g ** 2).item()
...
def federated_learning(
    ...
    client_loaders, _ = get_emnist_dataloaders(num_clients, batch_size, subset_size, alpha=alpha)
...
if __name__ == "__main__":
    ...
    print(f" → Accuracy={sos_accuracy:.2f}%, Time={sos_time:.2f}s\n")